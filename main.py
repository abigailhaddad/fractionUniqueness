...
def genDFOfNumeratorsAndDenominators(max_denominator):
   ...
   df=pd.DataFrame([[x, y] for x in range(0, max_denominator+1) for y in range(1, max_denominator+1) if y>=x])
...
def testValue(df, numerator, denominator):
    ...
    if numerator>denominator or denominator==0 or denominator>df['Denominator'].max():
        ...
        for digits in range(1, max_digits+1):
            ...
            all_options_values=[str(i[0])+"/" + str(i[1]) for i in all_options[['Numerator', 'Denominator']].values]
...
def deriveMaxDigits(df):
    # this takes the df and returns the number that corresponds to the maximum number of digits that we rounded to
